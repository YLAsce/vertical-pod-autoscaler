...
func TestAggregateStateByContainerName(t *testing.T) {
	...
	for _, c := range containers {
		assert.NoError(t, cluster.AddOrUpdateContainer(c, testRequest, 1))
	}
...
func TestAggregateContainerStateSaveToCheckpoint(t *testing.T) {
	...
	cs.AggregateCPUUsage.AddSample(1)
	cs.AggregateMemoryUsage.AddSample(1)
	cs.AggregateMemoryUsage.AddSample(10000000)
...
func TestAggregateContainerStateLoadFromCheckpoint(t *testing.T) {
	...
	// assert.False(t, cs.AggregateMemoryUsage.IsEmpty())
}
