...
func TestClusterAddSample(t *testing.T) {
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(testContainerID, testRequest, 1))
...
func TestMissingKeys(t *testing.T) {
	...
	err = cluster.AddOrUpdateContainer(testContainerID, testRequest, 1)
	assert.EqualError(t, err, "KeyError: {namespace-1 pod-1}")
}
...
func TestTwoPodsWithSameLabels(t *testing.T) {
	...
	err := cluster.AddOrUpdateContainer(containerID1, testRequest, 1)
	assert.NoError(t, err)
	err = cluster.AddOrUpdateContainer(containerID2, testRequest, 1)
	assert.NoError(t, err)
...
func TestEmptySelector(t *testing.T) {
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(containerID1, testRequest, 1))
	...
	assert.NoError(t, cluster.AddOrUpdateContainer(containerID2, testRequest, 1))
	...
	for _, tc := range cases {
		t.Run(tc.name+", VPA first", func(t *testing.T) {
			...
			for _, podDesc := range tc.pods {
				...
				assert.NoError(t, cluster.AddOrUpdateContainer(containerID, testRequest, 1))
			}
